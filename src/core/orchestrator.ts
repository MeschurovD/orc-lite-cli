...
export interface QueueRunOptions {
  configPath?: string
  queueIndex?: number      // 0-based; if omitted — first pending queue
  dryRun?: boolean
  cwd?: string
  signal?: AbortSignal
}

export async function runQueue(options: QueueRunOptions = {}): Promise<QueueResult> {
  ...
  const queue: QueueDefinition = config.queues[queueIdx]
  ...
  if (config.push !== 'none') pipelineLogger.info(`push mode: ${config.push}`)
  if (config.max_retries > 0) pipelineLogger.info(`max retries: ${config.max_retries}`)
  ...
  if (blockedTask) {
    throw new Error(
      `Queue ${queueLabel} is blocked at task "${blockedTask.file}" (status: ${blockedTask.status}).\n` +
  ...
  const signalHandler = async (sig: 'SIGINT' | 'SIGTERM') => {
  ...
  try {
    for (let i = 0; i < queue.tasks.length; i++) {
      ...
      const result = await runTask(
        task, i, queueIdx, freshConfig, resolvedConfigPath, cwd, total, notifier, projectName, queue.name,
      )
      currentTaskIndex = -1

      if (result.tokensUsed) totalTokensUsed += result.tokensUsed
...
export async function runAllQueues(options: Omit<QueueRunOptions, 'queueIndex'>): Promise<void> {
  const { config } = loadConfig(options.configPath)

  for (let i = 0; i < config.queues.length; i++) {
    const queue = config.queues[i]
    if (queue.status === 'done') continue
...
async function createAutoPr(
  ...
  if (prConfig.draft) args.push('--draft')
  ...
  try {
    ...
    pipelineLogger.error(`auto_pr failed: ${(err as Error).message}`)
  }
}
  ...
  for (let i = 0; i < queue.tasks.length; i++) {
    ...
    if (fileExists) {
      try {
        ...
        console.log(`     Prompt: ✗ build error: ${(err as Error).message}`)
      }
    }
...
async function checkAndHandleDirtyTree(
  git: GitService,
  cwd: string,
  configPath: string,
  config: ReturnType<typeof loadConfig>['config'],
