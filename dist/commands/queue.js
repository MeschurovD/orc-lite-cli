...
// ─── queue list ───────────────────────────────────────────────────────────────
export function queueListCommand(options) {
    let config;
    try {
...
// ─── queue add ────────────────────────────────────────────────────────────────
export async function queueAddCommand(name, options) {
    let config;
    let configPath;
    try {
        const result = loadConfig(options.config);
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function writeQueue(configPath, globalTasksDir, name, dir, schedule) {
    const raw = JSON.parse(readFileSync(configPath, 'utf-8'));
    const queues = Array.isArray(raw['queues']) ? raw['queues'] : [];
    const duplicate = queues.some((q) => typeof q === 'object' && q !== null && q['name'] === name);
    if (duplicate) {
