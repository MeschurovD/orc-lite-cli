...
const taskSchema = z.object({
    ...
    stages: z.array(z.enum(['implement', 'verify', 'test']))
        .refine((stages) => stages.length === 0 || stages[0] === 'implement', { message: 'stages[0] must be "implement"' })
        .optional(),
...
export function loadConfig(configPath) {
    ...
    if (data.queues && data.queues.length > 0) {
        ...
        queues = [{ name: 'default', schedule: null, status: 'pending', tasks: data.tasks }];
    }
    else {
...
export function updateTaskStatus(configPath, queueIndex, taskIndex, updates) {
...
export function updateQueueStatus(configPath, queueIndex, status) {
    const raw = JSON.parse(readFileSync(configPath, 'utf-8'));
    if (Array.isArray(raw['queues'])) {
        const queues = raw['queues'];
...
export function getTaskBranchName(task) {
    if (task.branch)
        return task.branch;
    const base = task.file.replace(/\.md$/i, '').replace(/[^a-zA-Z0-9_-]/g, '-');
    return `task/${base}`;
}
export function renderCommitMessage(template, vars) {
