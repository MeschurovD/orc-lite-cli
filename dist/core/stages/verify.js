...
export function parseVerifyOutput(output) {
    const newMatch = output.match(/\{[\s\S]*"approved"[\s\S]*"score"[\s\S]*\}/);
    if (newMatch) {
        try {
            return JSON.parse(newMatch[0]);
        }
        catch { /* fall through */ }
    }
...
function appendFullSummaryToTaskFile(taskFilePath, score, threshold, approved, fullSummary) {
    try {
...
export async function runVerifyStage(ctx) {
    ...
    const prompt = buildVerifyPrompt(taskContent, implementOutput || '(нет вывода реализации)', gitDiff || '(нет изменений)', stageConfig?.prompt_template);
